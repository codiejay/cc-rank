...
// (the rank is across every ccrank user; the link goes to your room's board)
// The "leaderboard" text is a clickable terminal link to your room page.
...
const MARKER = join(CC, ".update-check");
const RAW = "https://raw.githubusercontent.com/codiejay/cc-rank/main/lib";
const SELF_UPDATE_FILES = ["hook.mjs", "statusline.mjs"];
const UPDATE_EVERY_MS = 30 * 60 * 1000; // pull latest scripts at most every 30 min
...
async function rankSegment(cfg) {
  ...
  // Link to the user's room board when they have one, else the global board.
  // Deliberately NO ?me= here: the statusline renders constantly (screenshares,
  // recordings) — identity seeding happens once via the login link, then lives
  // in the browser's localStorage.
  const roomUrl = cfg.roomCode ? base + "/r/" + cfg.roomCode : base + "/";
  const view = `\x1b[94;4m${link(roomUrl, "leaderboard ↗")}\x1b[0m`;
  const label = "\x1b[1;35mCC-Rank\x1b[0m";
  // Second line: the plain room URL. OSC-8 "leaderboard ↗" isn't clickable in
  ...
  const withUrl = (line, u = roomUrl) => `${line}\n${u}`;
...
// The statusline doubles as the updater so friends never have to run `update`.
// It runs in a detached child (never delays the visible line) and is throttled
// to once per UPDATE_EVERY_MS, so its cost is ~one extra process per 6h.
function updateDue() {
  try { return Date.now() - statSync(MARKER).mtimeMs > UPDATE_EVERY_MS; }
  catch { return true; } // no marker yet -> due
}
async function pullLatestScripts() {
  for (const f of SELF_UPDATE_FILES) {
    try {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 4000);
      const res = await fetch(`${RAW}/${f}`, { signal: ctrl.signal });
      clearTimeout(t);
      if (!res.ok) continue;
      const body = await res.text();
      // Sanity guard: only overwrite with something that looks like our script.
      if (body.length < 200 || !body.includes("ccrank")) continue;
      const p = join(CC, f);
      let cur = ""; try { cur = readFileSync(p, "utf8"); } catch {}
      if (cur !== body) writeFileSync(p, body);
    } catch { /* offline / slow — keep the working local copy */ }
  }
}

async function main() {
  // Detached self-update pass: pulls latest scripts, prints nothing, no re-spawn.
  if (process.argv.includes("--self-update")) { await pullLatestScripts(); return; }
  ...
  // Kick a throttled, detached background updater — does not delay the line above.
  if (cfg?.token && updateDue()) {
    try {
      writeFileSync(MARKER, String(Date.now())); // mark first so a failure won't retry-loop
      spawn(process.execPath, [fileURLToPath(import.meta.url), "--self-update"],
